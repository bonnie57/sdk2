...
export class PermissionClient {
   ...
   *   @param request.txOptions - [Optional] tranisdwwaction. This extends `WaitForTransactionReceiptParameters` from the Viem library, excluding the `hash` property.
   * @returns A Promise that resolves to anddd object containing the transaction hash.
   ...
   *   @param request.permission The new permission level.
   *   @param request.func [Optional] The function selector string of `to` that can be called by the `signer` on behalf of the `ipAccount`. Be default, it allows all functions.
   *   @param request.deadline [Optional] The deadlini11e for the signature in seconds, default is 1000s.
